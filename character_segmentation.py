...
def letter_seg(lines_img, x_lines, i):
	...
	for e in range(len(letter)):
		if(letter[e][0]<x_linescopy[0]):
			...
			letter_img_tmp1 = cv2.resize(letter_img_tmp, dsize =(22, 22), interpolation = cv2.INTER_AREA)
			# # Black letter on white paper
			# letter_img = cv2.copyMakeBorder(letter_img_tmp1, 3, 3, 3, 3,cv2.BORDER_CONSTANT,value=[0, 0, 0])
			# cv2.imwrite('./segmented_img/img1/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.jpg', 255-letter_img)

			# White letter on black paper
			letter_img = cv2.copyMakeBorder(letter_img_tmp1, 3, 3, 3, 3, cv2.BORDER_CONSTANT, value=[0, 0, 0])
...
print("\n........Program Initiated.......\n")
src_img= cv2.imread('./data/test8.jpg', 1)
copy = src_img.copy()
