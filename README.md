...
​	主要工作是实现了一个简单的手写数字识别代码。简化的工作流程为：输入图像的预处理、字符分割、字符识别。
...
2. 字符分割
   ...
   ​	工作流程为：行分割、单词分割、字符分割。因此，要求输入图像的文本是按行分布的。

3. 字符识别

   ​	早期研究大多使用的是模板匹配，后期以特征提取为主。由于文字的位移，笔画的粗细，断笔，粘连，旋转等因素的影响，极大影响特征的提取的难度。

   ​	本代码采用卷积神经网络进行特征提取，数据集为 MNIST。在该数据集下，识别正确率约为 99.2%。
...
​	第五章为总结与展望，简述了所做的工作，并说明了可以继续研究和改进的问题。
...
​	对于预处理后的图像，统计每行像素值为 0 的像素的个数，得到如下图所示的颜色分布统计图。横轴表示图像的高，纵轴表示像素个数。统计图中有三个峰值，则表示文本中有三行。

​	理想的预处理图像是只有目标前景信息是白色的，背景部分完全是黑色的，直接在统计图的零点进行分行即可。然而由于图片噪声无法完全去除，得到的统计图也是混杂了许多噪点的。因此，需要设定一个阈值，在大于该阈值的部分进行分行处理。

![result.png](https://ooo.0o0.ooo/2017/06/06/5936697f2d29e.png)

### 3.2 分单词

​	首先，计算平均字符宽度。如果前一个字符与后一个字符之前的宽度大于平均字符宽度，可以认为这两个字符分别输入两个单词，则在这个字符进行单词分割。

​	输入图片的平均字符宽度为 25.8px，单词分割后的结果如下图所示。

![word.png](https://ooo.0o0.ooo/2017/06/06/593670e642c38.png)
...
​	轮廓检测结果为：                                                        <img src="https://ooo.0o0.ooo/2017/06/04/5933c6d0bef26.png" alt="contours.png" title="contours.png" style="zoom:40%"/>

​	字符分割结果为：

<img src="https://ooo.0o0.ooo/2017/06/04/5933f965c6b1a.png" title="contours.png" style="zoom:60%"/>
...
![test9.JPG](https://ooo.0o0.ooo/2017/06/06/593659da53b89.jpg)

2. 进行字符分割

   ![example.png](https://ooo.0o0.ooo/2017/06/06/59365be9a5b1e.png)

   3. 识别结果

      ![output.png](https://ooo.0o0.ooo/2017/06/06/59365ca8534ba.png)



## 第五章 总结与展望

​	主要存在的问题是对输入图片的要求很高，不适用于一般环境。后期可以在以下部分做出改进。

1. 倾斜矫正

   ​	对文档进行拍照时一般是比较随意的，即使写的时候按照行对齐来写的，在拍照的时候也会出现倾斜的情况，影响字符的分割。因此，在图片预处理时，需要加入倾斜矫正的步骤。

2. 版面还原

   ​	即要求识别后的字符仍然按照原文档图片那样排列着，位置不变，顺序不变。

3. 扩展到字母识别

   ​	工作流程于数字识别相同，主要问题在于训练卷积神经网络的数据集。